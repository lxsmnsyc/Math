...
function vec2.__eq(a, b)
    assertParams(isvec(a) or isnum(a), "__eq", "a", "is not a vec2")
    assertParams(isvec(b) or isnum(b), "__eq", "b", "is not a vec2")
    if(isvec(a)) then
        if(isvec(b)) then 
...
function vec2.__lt(a, b)
    assertParams(isvec(a) or isnum(a), "__lt", "a", "is not a vec2")
    assertParams(isvec(b) or isnum(b), "__lt", "b", "is not a vec2")
    if(isvec(a)) then
        if(isvec(b)) then 
...
function vec2.__le(a, b)
    assertParams(isvec(a) or isnum(a), "__le", "a", "is not a vec2")
    assertParams(isvec(b) or isnum(b), "__le", "b", "is not a vec2")
    if(isvec(a)) then
        if(isvec(b)) then 
            return a.x <= b.x and a.y <= b.y
        elseif(isnum(b)) then
