...
function vec4.__eq(a, b)
    assertParams(isvec(a) or isnum(a), "__eq", "a", "is not a vec4")
    assertParams(isvec(b) or isnum(b), "__eq", "b", "is not a vec4")
    if(isvec(a)) then
        if(isvec(b)) then 
...
function vec4.__lt(a, b)
    assertParams(isvec(a) or isnum(a), "__lt", "a", "is not a vec4")
    assertParams(isvec(b) or isnum(b), "__lt", "b", "is not a vec4")
    if(isvec(a)) then
        if(isvec(b)) then 
...
function vec4.__le(a, b)
    assertParams(isvec(a) or isnum(a), "__le", "a", "is not a vec4")
    assertParams(isvec(b) or isnum(b), "__le", "b", "is not a vec4")
    if(isvec(a)) then
        if(isvec(b)) then 
            return a.x <= b.x and a.y <= b.y and a.z <= b.z and a.w <= b.w
        elseif(isnum(b)) then
